...
@section('page_title', 'إضافة منتج طبي جديد | Add New Product')
        ...
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="name_ar" id="name_ar" required value="{{ old('name_ar') }}"
                           placeholder="مثال: جهاز قياس الضغط الرقمي..." 
                ...
                <div class="space-y-1.5" dir="ltr">
                    ...
                    <input type="text" name="name_en" id="name_en" required value="{{ old('name_en') }}"
                           placeholder="Example: Digital Blood Pressure Monitor..." 
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="details_ar" id="details_ar" rows="4" 
                          placeholder="المواصفات الفنية، القياسات، كيفية الاستخدام باللغة العربية..." 
                          class="w-full bg-slate-50 border border-slate-100 focus:border-emerald-500 focus:bg-white rounded-xl px-4 py-3 text-slate-900 text-sm focus:outline-hidden transition-all-300">{{ old('details_ar') }}</textarea>
                @error('details_ar')
                    <span class="text-rose-500 text-xxs font-semibold block mt-1">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="space-y-1.5" dir="ltr">
                ...
                <textarea name="details_en" id="details_en" rows="4" 
                          placeholder="Technical details, components, warnings in English..." 
                          class="w-full bg-slate-50 border border-slate-100 focus:border-emerald-500 focus:bg-white rounded-xl px-4 py-3 text-slate-900 text-sm focus:outline-hidden transition-all-300 text-left">{{ old('details_en') }}</textarea>
                @error('details_en')
                    <span class="text-rose-500 text-xxs font-semibold block mt-1 text-right">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="space-y-4 bg-slate-50/50 border border-slate-100 p-5 rounded-2xl">
                <h3 class="font-bold text-slate-800 text-xs flex items-center gap-2">
                    ...
                    <span>صورة المنتج الرئيسية (Product Image)</span>
                </h3>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <!-- File upload -->
                    <div class="space-y-1.5">
                        <label for="image_file" class="block text-xxs font-semibold text-slate-500">رفع صورة من جهازك / Upload Image</label>
                    ...
                    <!-- Image URL -->
                    <div class="space-y-1.5">
                        <label for="image_url" class="block text-xxs font-semibold text-slate-500">أو رابط صورة خارجي / Or External URL</label>
            ...
            <button type="submit" class="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-8 py-3.5 rounded-xl shadow-md shadow-emerald-600/10 transition-all-300 text-sm">
                حفظ وإضافة المنتج / Save Product
            </button>
        </form>
    </div>
@endsection
