...
@section('content')
    ...
    @if($categories->isEmpty())
        <div class="bg-white border border-slate-100 rounded-2xl shadow-sm p-12 text-center text-slate-400">
            ...
            <p class="text-sm font-semibold">لا توجد أقسام مسجلة في قاعدة البيانات حالياً.</p>
        </div>
    @else
            ...
            @foreach($categories as $category)
                <div class="bg-white border border-slate-100 rounded-2xl shadow-sm p-4 flex items-center gap-4">
                    ...
                    <div class="w-14 h-14 rounded-xl bg-slate-50 border border-slate-100 overflow-hidden flex items-center justify-center shrink-0">
                        @if($category->image)
                            <img src="{{ $category->image }}" alt="{{ $category->name }}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                    ...
                    <div class="min-w-0 flex-grow">
                        <p class="font-black text-slate-900 text-sm">{{ $category->name }}</p>
                        <p class="text-[11px] text-slate-400 font-mono mt-0.5 truncate" dir="ltr">{{ $category->slug }}</p>
                        @if($category->description)
                            <p class="text-xs text-slate-500 mt-1 line-clamp-1">{{ $category->description }}</p>
                        @endif
                    </div>
                    {{-- Count + Actions --}}
                            ...
                            <tr class="hover:bg-slate-50/50 transition-all">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.categories.destroy', $category->id) }}" method="POST"
                                              class="inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا القسم؟ سيتم حذف جميع المنتجات المرتبطة به!')">
                                            @csrf @method('DELETE')
                                            <button type="submit"
        ...
        @if($categories->hasPages())
            <div class="mt-4 lg:hidden">{{ $categories->links() }}</div>
        @endif
